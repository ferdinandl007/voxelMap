...
public class OctreeNode<T: Equatable>: CustomStringConvertible {
    ...
    private func subdivide(adding element: T, at point: vector_double3) -> OctreeNode? {
        precondition(elements != nil, "Subdividing while leaf does not contain a element")
        ...
        case .leaf:
            ...
            add(elements, at: self.point)
            elements = nil
            self.point = nil
            // add new element to children
            return add(element, at: point)
        case .internal:
...
public class Octree<T: Equatable>: CustomStringConvertible {
    ...
    public init(boundingBox: Box, minimumCellSize _: Double) {
